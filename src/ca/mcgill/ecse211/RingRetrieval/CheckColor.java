...
/**
 * Class used to detect ring assuming it at the tree. At each side, it tries to
 * detect and grab ring. Method in this classed are called by Ev3Boot class.
 * 
...
public class CheckColor extends MotorController {
	...
	private static double minimum_value = 0.013;
	...
	public static void restartChecker() {
		detected = false;
		colorCount = new int[][] { { 0, 0, 0, 0 }, { 0, 0, 0, 0 } };
	}
	 ...
	 * 
	 * @return 1 for low elevation, 2 for high elevation, 0 if no colour has been
	 *         detected.
	 */
	...
	/**
	 * This method assumes the robot is close enough to the ring, It makes the robot
	 * slightly move around to record multiple samples using the fetchColor method.
	...
	/**
	 * This method records multiple samples, and implements a filter to ignore
	 * values too low caused by ambient lighting.
	 */
	private static void fetchColor(int elevation) {
		...
		do {
			...
			double distance = Math.sqrt(colorBuffer[0] * colorBuffer[0] + colorBuffer[1] * colorBuffer[1]
					+ colorBuffer[2] * colorBuffer[2]);
			...
			if (distance > minimum_value) {
				...
				for (int i = 0; i < 4; i++) {
					...
					double error = Math.pow(data[0] - COLOR[i][0][0], 2) + Math.pow(data[1] - COLOR[i][0][1], 2)
